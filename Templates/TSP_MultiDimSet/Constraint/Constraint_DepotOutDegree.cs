...
{
    /// <summary>
    /// [C3] DepotOutDegree ∀ depot ∈ DEPOT：
    /// Σ_{(depot,j) ∈ ARC} UseArc[depot][j] = 1
    /// </summary>
    public sealed class Constraint_DepotOutDegree : ConstraintBase
    {
        private readonly List<Set_Depot> _depots;
        private readonly List<Set_Arc> _arcs;

        public Constraint_DepotOutDegree(
        ...
        public void Build(OptEngine engine)
        {
            ...
            {
                foreach (var arc in _arcs.Where(arc => arc.From == depot))
