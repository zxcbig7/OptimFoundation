...
namespace FJSP_BASIC.Data
{
    public class Dataload
    {
        ...
        public List<string> Scope = new List<string> { "Total" }; // Makespan scalar 用單一成員 set（建模 artifact，非資料）
        ...
        // 預設資料來源＝CSV：讀 Data/Parameter_ProcessTime.csv（foundation CsvCtrl.BuildParameter，表頭按名對位）。
        ...
        // Set 改由檔案定義後可能與 Parameter 失同步，載入即 fail fast：parameter 出現的值必須 ⊆ 對應 set
        private void ValidateSetsCoverParameters()
        {
            var missing = new List<string>();
            missing.AddRange(parameter_ProcessTime.Select(p => p.Lot).Distinct().Except(LotSet).Select(v => $"Lot '{v}'"));
            missing.AddRange(parameter_ProcessTime.Select(p => p.Operation).Distinct().Except(OperationSet).Select(v => $"Operation '{v}'"));
            missing.AddRange(parameter_ProcessTime.Select(p => p.Eqp).Distinct().Except(EqpSet).Select(v => $"Eqp '{v}'"));
            if (missing.Count > 0)
        ...
        private void VerifySolution(OptEngine engine, double makespan, double objective, double penaltyCost)
        {
            ...
            foreach (var lot in LotSet)
            {
                foreach (var op in OperationSet)
                {
                    ...
                    // AssignOneEqp：恰好指派一台
                    var eqps = EqpSet.Where(e => assignVals[$"VariableB_Assign@{lot}@{op}@{e}"] > 0.5).ToList();
                ...
                for (int i = 0; i + 1 < OperationSet.Count; i++)
                    Check(start[(lot, OperationSet[i + 1])] >= complete[(lot, OperationSet[i])] - eps,
                        $"RoutePrecedence@{lot}@{OperationSet[i]}→{OperationSet[i + 1]}");

                // MakespanDef：Makespan ≥ 最後一道完成
                Check(makespan >= complete[(lot, OperationSet[OperationSet.Count - 1])] - eps, $"MakespanDef@{lot}");
            }
